...
// ---- finding ingest (SAST/secret-style; POST /api/ci/finding) --------------

/// A single finding (mirrors the Java Finding model; nulls omitted).
#[derive(Serialize, Default, Clone)]
#[serde(rename_all = "camelCase")]
pub struct Finding {
    pub identity: String,
...
impl WardenClient {
    /// POST /api/ci/finding — upload SAST/secret-style findings.
    pub fn upload_findings(&self, scan_id: &str, findings: &[Finding]) -> Result<(), String> {
        let body = FindingUpload { scan_id, findings, strategy: "AllFiles" };
        self.send_empty("POST", "/api/ci/finding", &body)?;
        println!("[warden] uploaded {} finding(s)", findings.len());
        Ok(())
    }
}
